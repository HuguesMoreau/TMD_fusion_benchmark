...
- **external_repositories** contains files from other repositories that I copied here for convenience: [SVCCA](https://github.com/google/svcca/blob/master) and [deepCCA](https://github.com/Michaelvll/DeepCCA). I do not claim autorship of any of the files in this directory. Both repositories were used to try to produce networks that were forced to produce complementary features. As these networks did not have better accuracy than the others, we did not talk about them in the publication, but the code is available in `architecture\dissimilar_nets.py`. 
